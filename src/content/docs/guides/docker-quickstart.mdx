...
From then on it snapshots the `app-data` volume — the SurrealDB store and wiki assets, i.e. the
...
aimed at hardcode development rather than at hosting a game. They use ArangoDB and expose the
metrics endpoints, and they expect the .NET toolchain described in
