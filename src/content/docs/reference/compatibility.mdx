...
<Card title="Forgiving Parenthesis">

In PennMUSH, the following is legal, while is not in SharpMUSH:

```lisp title="PennMUSH" frame="terminal" "> #1"
lsearch(all,eval,\[strmatch(##,%#)\])
> #1
```

Instead, escape all the contents. This is also needed for #lambda evaluation.
```lisp title="SharpMUSH" frame="terminal" "> #1"
lsearch(all,eval,\[strmatch\(##\,%#\)\])
> #1
```
...
```lisp title="SharpMUSH" frame="terminal" "> [if(true,red,white)]"
iter(\[if(true\,red\,white)\],##,)
> [if(true,red,white)]
```

If double-evaluation is needed, wrap the expression explicitly with `[##]`.
